...
%%%%%%%% Better implementation in terms of time...

%%%%%%%Sampling the whole timestep one time and using it to find PC coefficients

%%%%%%use assemble_stiffness_new for this
...
K_ss = assemble_stiffness_New(ne,nN,K,xi(1,ss),mu_g,sigma_g,x,l,psi,ss,n_inpce,expnsn);
...
for ipce = 1:n_outpce
    ...
    for ss = 1:ns
       ...
       U_MCS_sum(:,:,ss) =  U_MCS(:,:,ss) * psi(ss,ipce);

    end

    
u_numerator = sum(U_MCS_sum,3)/ns;
