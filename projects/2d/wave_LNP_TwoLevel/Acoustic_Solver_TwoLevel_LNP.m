...
% %%%% SSFEM - Domain Decomposition - Acoustic Wave Propagation Problem - Random Process -Two level %
...
[p_f,e_f,t_f,GLnode,npart,point_exact,triangle_exact,node_gamma,node_interior,...
    node_interfacewhole,node_interiorwhole,R,D,node_corner,node_remaining,node_c,node_r,Rc,Rr,Bc,...
    node_gamma_OG,node_interior_OG,node_corner_OG,node_remaining_OG]...
    = ExtractGmshPart_puff_TwoLevel('./square.msh');
...
num_interior = size(nonzeros(unique(node_interiorwhole)),1);
...
Assemble_Stochastic(s_spectral,Cijk,npart,node_gamma,node_interior,node_c,node_r,num_interface,...
...
iter = zeros(length(node_interfacewhole)*n_outpce,count+1);
nres = zeros(length(node_interfacewhole)*n_outpce,count+1);
%%%%%
...
ktmat = load('K_T_gg.mat');
 K_T_gg = ktmat.K_T_gamma;
 
ktmat = load('K_T_gi.mat');
 K_T_gi = ktmat.K_T_gintr;
 
 ktmat = load('K_T_ii.mat');
 K_T_ii = ktmat.K_T_intr;
...
G_local = b_t_interface - (K_T_gi * (K_T_ii \ b_t_interior));
...
U_gamma_o = zeros(length(node_interfacewhole)*n_outpce,1);
r_gamma_o = G_global;
...
[U,iternum,res] = solve_NeumannTwoLevel(U_gamma_o,r_gamma_o,node_gamma,node_interior,...
