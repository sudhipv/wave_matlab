...
function [U] = DoPCGMLumpedTwoLevel(npart,U_o,d_global,tol)

r_gamma_o = d_global;

z_o = GetPreconditionedZLumped(npart,r_gamma_o);
...
for j = 1:n
   ...
   Q(:,:,j) = GetMatrixVectorProductTwoLevel(npart,P(:,:,j)); 
  ...
  z(:,:,j+1) = GetPreconditionedZLumped(npart,r_gamma(:,:,j+1));
