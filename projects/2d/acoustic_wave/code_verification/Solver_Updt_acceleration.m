...
% Assemble vector
b = AssembleVector(p, e, t, c, 4, 'Acoustic', [], time(i));
...
figure(10)
u_fenics = load('./data/u_fenics.mat');
tt = load('./data/tt.mat');
plot(time,SOL(pos,:),time,U(pos,:),tt.tt,u_fenics.u_fenics(pos,:));
...
E_norm = zeros(nt,1);
for kk = 1:nt
E_norm(kk) = norm(SOL(:,kk) - U(:,kk))/norm(SOL(:,kk));
end
figure(12)
plot(time,E_norm);
