function [A] = AssembleMatrix_ss(points, edges, triangles,eq, pde, W, time, s_spectral,xi,file)
...
% Create matrix
A = zeros(size(points,2),size(points,2));
...
for triangle = triangles
  ...
  for p = midpoints
	...
	integral = feval(pde, u, v, w, du, dv, dw, dx, 0, x, d, time, eq, s_spectral,xi,file) / 3.0;
...
% % %   d = edge(5);
  ...
  for p = gausspoints
	...
	integral = feval(pde, u, v, w, [0;0], [0;0], dw, 0, ds, x, d, time, eq, s_spectral,xi,file) / 2.0;
	A(nodes(i),nodes(j)) = A(nodes(i),nodes(j)) + integral;
