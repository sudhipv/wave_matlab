...
function [U,iter,nres] = DoPCGMLumped(npart,U_o,d_global,node_interface_part,node_interior_part,R,D,tol,count,NBcount)

r_gamma_o = d_global;

Z_o = GetPreconditionedZLumped(npart,node_interface_part,r_gamma_o,R,D);
...
%    fprintf('PCGM Iteration %d\n',j);
  ...
  Z(:,:,j+1) = GetPreconditionedZLumped(npart,node_interface_part,r_gamma(:,:,j+1),R,D);
