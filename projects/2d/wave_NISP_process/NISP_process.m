...
%%%%%%%%% NISP Process - Acoustic Wave Equation %%%%%%%%%%%%%
...
cd ../../../ssfem_wave/wave_NISP_process
...
U_NISP = zeros(size(p,2),n_outpce);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
U_wave = zeros(length(p(1,:)),count+1);
U_std = zeros(length(p(1,:)),count+1);
...
% % % % Assemble vector
b = AssembleVector(p, e, t, 4, 'Acoustic_ss', [], time(i));
...
% Assemble matrix
A_NISP(:,:) = AssembleMatrix_ss(p, e, t,1, 'Acoustic_ss', [], 0, s_spectral,xi(np,:),file);

K_transient = (M/(beta*deltaT^2)) + A_NISP;

b_n = (b_n.*psi(np,n_pc)*w(np));
    
sum_numerator = sum_numerator + K_transient\b_n;


end

U_NISP(:,n_pc) = (sum_numerator)/sqrt(norm.norm_squared(n_pc));
