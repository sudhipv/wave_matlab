...
[p_f,e_f,t_f,GLnode,npart,point_exact,triangle_exact,node_gamma,node_interior,...
 node_interfacewhole,node_remainingwhole,R,D,node_corner,node_remaining,node_c,...
 node_r,Rc,Rr,Bc,node_gamma_OG,node_interior_OG,node_corner_OG,node_remaining_OG] ...
...
iter = zeros(length(node_interfacewhole),count+1);
nres = zeros(length(node_interfacewhole),count+1);
%%%%%
...
if(NBcount ==2)
    strcond = 'Condition number of K_T for subdomain %d is %d \n';
    fprintf(strcond,i,cond(K_T))
end
...
% [U_gamma,iternum,res] = DoPCGMNeumann(npart,U_gamma_o,r_gamma_o,node_gamma,node_interior,R,D,tol,count,NBcount);

[U_gamma,iternum,res] = DoPCGM_TwoLevelNeumann(npart,node_c,node_r,...
   node_corner,node_gamma,node_interior,U_gamma_o,r_gamma_o,Rc,Rr,Bc,R,D,tol,tolc);
