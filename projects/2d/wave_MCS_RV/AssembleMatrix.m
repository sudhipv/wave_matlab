function A = AssembleMatrix(points, edges, triangles, eq, pde, W, time)
...
for triangle = triangles
  ...
  for p = midpoints
	...
	integral = feval(pde, u, v, w, du, dv, dw, dx, 0, x, d, time, eq) / 3.0;
...
% % %   d = edge(5);
  ...
  for p = gausspoints
	...
	integral = feval(pde, u, v, w, [0;0], [0;0], dw, 0, ds, x, d, time, eq) / 2.0;
	A(nodes(i),nodes(j)) = A(nodes(i),nodes(j)) + integral;
